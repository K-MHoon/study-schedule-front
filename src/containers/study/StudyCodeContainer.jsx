...
import { createInviteStudyCode, removeStudyCodeList } from '../../lib/api';
...
const StudyCodeContainer = () => {
  ...
  const removeCode = async (studyCodeList) => {
    try {
      await removeStudyCodeList(studyId, studyCodeList);
      alert('발급된 코드가 삭제되었습니다.');
      window.location.reload();
    } catch (e) {
      console.log(e);
      if (e.response.status === 504) {
